...
const PaginationSong = ({totalPostsSong, postsPerPage }) => {
  const { result, dispatch } = React.useContext(GuitarKeContext);
  let pattern = [];
  var range_page = Math.ceil(totalPostsSong / postsPerPage); 
  ...
  if (range_page <= 5) {
    ...
    switch (true) {
      ...
      case result.page_num > range_page - 4:
        pattern = [1, "...", range_page - 3, range_page - 2, range_page - 1, range_page];
        break;
      default:
        pattern = [
          1,
